...
controller('MainCtrl', ['$scope', 'checkMobile', '$timeout',
  ...
  function ($scope, checkMobile, $timeout) {
    ...
    socket.on('game', function (GAME) {
      ...
      $scope.words = _.uniq(['abc', 'def', 'asdf', 'asdfasdg', 'zxcvxzcv', 'utuytuyu', 'xxqweqe', 'piuouio', 'tyuiyuix', 'asdfasdfb'])
      $scope.$apply()
    })
    ...
    function timer() {
      ...
      $scope.$apply(function () {
        ...
        $scope.timeLeft = Math.ceil(time/1000)
        var ms = zeroPad(Math.floor(time % 1000 / 10))
        time /= 1000
        var sec = zeroPad(Math.ceil(time % 60))
        time /= 60
